...
var checkerEmail = require("email-validator");
...
function postStudents(req,res){
    ...
    if(validateEmail.test(email) && validateCpf.length == 11 && !isNaN(validateCpf)){
        ...
        .then((students)=>{
            if(!students){
            ...
            }else if(name == undefined || email == undefined || cpf == undefined || undefined || checkerCpf(cpf) == false || checkerEmail.validate(email) == false){
                     res.statusCode = 400;
                     res.send("PT - Preencha todos os campos / EN - Fill in all fields");
                ...
                .then(()=>{
                    ...
                    let transportation = nodeMailer.createTransport({
                        host: "smtp-mail.outlook.com",
                        port: 587,
                        secure: false,
                        auth:{
                            user: "modulo_aluno@hotmail.com",
                            pass: "Modulo<4"
                        }
                    });

                    transportation.sendMail({
                        from: "Student_Module <modulo_aluno@hotmail.com>",
                        to: `${email}`,
                        subject: "Cadastro Aluno realizado com sucesso!",
                        html: `Parabéns seu cadastro foi criado com sucesso: ${email} - ${name}`
                    }).then(()=>{
                        console.log("Email enviado com sucesso!")
                    })
                    .catch((failed)=>{
                        console.log("Erro ao enviar confirmação de email: " + failed)
                    })
